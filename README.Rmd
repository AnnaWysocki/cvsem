...
The **cvsem** package provides cross-validation (CV) of structural equation models (SEM) across a user-defined number of folds [@Cudeck1983]. CV fits individual models via the **lavaan** package [@Rosseel2012lavaan] and compares the test covariance matrix to the implied covariance matrix from the training data for each fold via a prespecified distance metric (defults to Kullback-Leibler divergence). The `cvsem` function returns the average distance metric with a corresponding standard error for each tested model.  
...
Here we use `k=10` folds. CV is based on the distance between test sample covariance matrix and the model implied matrix from the training data. The distance among sample and implied matrix is defined in `distanceMetric`. Here we use `KL-Divergence`. 
```{r}
fit <- cvsem( x = example_data, Models = models, k = 10, distanceMetric = "KL-Divergence")
```

## Show Results
Print cv-object. Note, model with smallest (best) distance metric is listed first. 
```{r}
fit
```
